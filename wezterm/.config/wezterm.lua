...
-- ─────────────────────────────────────────────────────────────
-- Section 1: Helpers & Environment Classification
-- ─────────────────────────────────────────────────────────────

local ENV_PROD = 'prod'
local ENV_STAGING = 'staging'
local ENV_LOCAL = 'local'

local function classify_environment(domain_name)
  if domain_name:find('prod') then return ENV_PROD end
  if domain_name:find('staging') then return ENV_STAGING end
  return ENV_LOCAL
end

local ENV_STYLES = {
  [ENV_PROD]    = { color = '#e05561', label = ' PROD',  tab_bg = '#3d1014' },
  [ENV_STAGING] = { color = '#d4a04a', label = ' STG',   tab_bg = '#3d3014' },
  [ENV_LOCAL]   = { color = nil,       label = nil,      tab_bg = nil },
}
...
-- Disable WezTerm's built-in libssh-rs domains (auto-detected from ~/.ssh/config).
-- libssh-rs doesn't support ProxyCommand, so prod hosts (cloudflared) fail.
-- Our exec domains below shell out to real `ssh` which reads ~/.ssh/config properly.
config.ssh_domains = {}

config.exec_domains = {}
for _, host in ipairs(ssh_hosts) do
  table.insert(config.exec_domains, wezterm.exec_domain('ssh:' .. host, function(cmd)
    cmd.args = { 'ssh', host }
    return cmd
  end))
end
...
wezterm.on('format-tab-title', function(tab, tabs, panes, conf, hover, max_width)
  ...
  -- Environment from domain
  local domain_name = pane.domain_name or ''
  local env = classify_environment(domain_name)
  local style = ENV_STYLES[env]

  -- Tab index (1-based, matches CMD+N shortcuts)
  local index = tab.tab_index + 1

  -- Every tab is a pill — active is prominent, inactive is subtle.
  -- This prevents layout shift when switching tabs.
  local pill_bg
  if style.tab_bg then
    pill_bg = style.tab_bg                          -- env color (red/amber)
  elseif tab.is_active then
    pill_bg = '#2a2a2a'                             -- active: visible
  elseif hover then
    pill_bg = '#1e1e1e'                             -- hover: hint
  else
    pill_bg = '#161616'                             -- inactive: barely there
  end
  ...
  -- Environment label in env color
  if style.color and style.label then
    table.insert(elements, { Foreground = { Color = style.color } })
    table.insert(elements, { Text = style.label })
  end
...
wezterm.on('update-right-status', function(window, pane)
  ...
  -- Environment badge (if SSH domain)
...
config.keys = {
  ...
  -- SSH host picker
  {
    ...
    action = act.ShowLauncherArgs { flags = 'FUZZY|DOMAINS', title = 'SSH Hosts' },
  },
...
config.launch_menu = {
  ...
  -- Staging
  { label = '── Staging ──' },
  { label = '  fe-staging',       domain = { DomainName = 'ssh:fe-staging' } },
  { label = '  be-staging',       domain = { DomainName = 'ssh:be-staging' } },

  -- Production
  { label = '── Production ──' },
  { label = '  vision-prod',      domain = { DomainName = 'ssh:vision-prod' } },
  { label = '  mycohort-prod',    domain = { DomainName = 'ssh:mycohort-prod' } },
  { label = '  cohort-live-prod', domain = { DomainName = 'ssh:cohort-live-prod' } },
}
...
wezterm.on('augment-command-palette', function(window, pane)
  ...
  for _, host in ipairs(ssh_hosts) do
    table.insert(commands, {
      brief = 'SSH: ' .. host,
      action = act.SpawnTab { DomainName = 'ssh:' .. host },
    })
  end
